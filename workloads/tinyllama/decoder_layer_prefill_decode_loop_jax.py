...
def attention_decode_masked(q, k, v, valid_length):
    num_heads = q.shape[1]
    num_kv_heads = k.shape[1]
    ...
    key_positions = jnp.arange(cache_length, dtype=jnp.int32)
    valid_mask = key_positions < valid_length
    scores = jnp.where(valid_mask[None, None, None, :], scores, -jnp.inf)
...
def decoder_layer_prefill_decode_loop(
    ...
    decode_steps = decode_cos.shape[0]
    ...
    decode_inputs = (
        decode_cos,
        decode_sin,
        cache_position + jnp.arange(decode_steps, dtype=jnp.int32),
    )

    def decode_step(loop_state, step_inputs):
        k_cache, v_cache, decode_hidden_state = loop_state
        step_cos, step_sin, step_cache_position = step_inputs

        step_cos = step_cos[None, :]
        step_sin = step_sin[None, :]
        ...
        decode_q, decode_k = apply_rope(decode_q, decode_k, step_cos, step_sin)
        ...
        valid_length = step_cache_position + jnp.array(1, dtype=jnp.int32)
        decode_context = attention_decode_masked(
            decode_q,
            k_cache,
            v_cache,
            valid_length,
        ...
        logits, next_token_id, next_decode_hidden_state = (
            generation_tail(
                decode_output,
                final_norm_weight,
                lm_head_weight,
        ...
        return (k_cache, v_cache, next_decode_hidden_state), step_output

    (k_cache, v_cache, _), (decode_outputs, logits, next_token_ids) = jax.lax.scan(
        decode_step,
        (k_cache, v_cache, initial_decode_hidden_state),
        decode_inputs,
    )
    decode_outputs = jnp.transpose(decode_outputs, (1, 0, 2))
    logits = jnp.transpose(logits, (1, 0, 2))
    next_token_ids = jnp.transpose(next_token_ids, (1, 0))

    return prefill_output, decode_outputs, logits, next_token_ids, k_cache, v_cache
...
def main():
    ...
    initial_decode_hidden_state = jnp.ones((batch, 1, hidden), dtype=jnp.float32)
    ...
    decode_cos = jnp.ones((decode_steps, head_dim), dtype=jnp.float32)
    decode_sin = jnp.ones((decode_steps, head_dim), dtype=jnp.float32)
    ...
    lowered = jax.jit(decoder_layer_prefill_decode_loop).lower(
        prefill_hidden_states,
        initial_decode_hidden_state,
        input_norm_weight,
        post_norm_weight,
